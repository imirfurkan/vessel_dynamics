...
struct MPCParams
{
  unsigned int    P_;       // Prediction horizon (number of steps)
  ...
  Eigen::Vector3d tau_min_; // Minimum thruster force/torque
  Eigen::Vector3d tau_max_; // Maximum thruster force/torque
};
...
struct MPCState
{
  ...
  // The numerical matrices for the augmented model p[k+1] = A_bar*p[k] + B_bar*delta_tau[k]
...
struct solverObjects
{
  ...
  casadi::Function              solver_; // The compiled solver function
  ...
  casadi::MX pref_param_;     // The reference for the output y (part of p)
